-- Script SQL para inserir os 4 posts principais do blog XZenPress
-- Execute este script no SQL Editor do Supabase

-- POST 1: 5 Pontos de Acupressão para Aliviar o Estresse Diário
...
*Experimente estes pontos na nossa plataforma XZenPress com timer integrado e cromoterapia!*',
  ...
  'acupressao',
  ARRAY['acupressão', 'estresse', 'ansiedade', 'medicina-tradicional-chinesa'],
  true,
  NOW() - INTERVAL '2 days',
  1247,
  8
)
ON CONFLICT (lesma) DO UPDATE SET
  titulo = EXCLUDED.titulo,
  contente = EXCLUDED.contente,
  trecho = EXCLUDED.trecho,
  atualizado_em = NOW();

-- POST 2: Respiração 4-7-8: A Técnica Científica para Ansiedade
INSERT INTO posts_do_blog (
  titulo,
  lesma,
  contente,
...
*Experimente agora na nossa plataforma com timer visual e cromoterapia integrada!*',
  ...
  'Equipe XZenPress',
  'aleksayevacupress@gmail.com',
  'https://dqjcbwjqrenubdzalicy.supabase.co/storage/v1/object/public/acupressure-images/Logo-Xzenpress-oficial.png',
  'respiracao',
  ARRAY['respiração', '4-7-8', 'ansiedade', 'ciência', 'bem-estar'],
  true,
  NOW() - INTERVAL '5 days',
  892,
  6
...
-- POST 3: Lei 14.831/2024: Como Sua Empresa Pode Se Tornar Promotora da Saúde Mental
INSERT INTO posts_do_blog (
  titulo,
  lesma,
  contente,
...
*Quer saber como implementar? Entre em contato conosco para uma consultoria personalizada!*',
  ...
  ARRAY['lei-14831', 'saúde-mental', 'corporativo', 'compliance', 'nr-1'],
  true,
  NOW() - INTERVAL '1 day',
  634,
  12
...
-- POST 4: Cromoterapia: Como as Cores Afetam Nosso Bem-estar
INSERT INTO posts_do_blog (
  titulo,
  lesma,
  contente,
...
*Experimente a cromoterapia integrada na nossa plataforma XZenPress!*',
  ...
  'https://dqjcbwjqrenubdzalicy.supabase.co/storage/v1/object/public/acupressure-images/Logo-Xzenpress-oficial.png',
  'cromoterapia',
  ARRAY['cromoterapia', 'cores', 'terapia', 'neurociência', 'bem-estar'],
  true,
  NOW() - INTERVAL '7 days',
  456,
  10
...
SELECT 'Posts inseridos com sucesso! Agora você tem 5 posts no total no blog.' AS mensagem;
