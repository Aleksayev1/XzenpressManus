...
self.addEventListener('fetch', event => {
  ...
  // Skip range requests (crucial for Audio/Video elements without 206 Partial Content support in SW)
  if (request.headers.has('range')) return;

  // Skip chrome-extension, Google Translate and other protocols
  if (!url.protocol.startsWith('http')) return;
  if (url.hostname.includes('translate.google.com')) return;
  if (url.hostname.includes('translate.googleapis.com')) return;
  ...
  event.respondWith(
    (async () => {
      ...
      if (!finalResponse) {
        // Fallback final para evitar TypeError
