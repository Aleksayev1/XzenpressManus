...
import { BookOpen, Calendar, User, Eye, Clock, Search, Filter, ArrowRight, ArrowLeft } from 'lucide-react';
...
export const BlogPage: React.FC<BlogPageProps> = ({ onPageChange }) => {
  ...
  // Convert markdown to HTML with embedded HTML support
  const processMarkdown = async (markdown: string): Promise<string> => {
    const result = await unified()
      .use(remarkParse)
      .use(remarkGfm)
      .use(remarkHtml, { sanitize: false }) // Allow raw HTML for YouTube embeds
      .process(markdown);
  ...
  // Process markdown content when selectedPost changes
  useEffect(() => {
    if (selectedPost) {
      const content = getTranslatedField(selectedPost, 'content');
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                {filteredPosts.map((post) => (
                  ...
                  >
                    {/* Featured Image */}
                    {post.imageUrl && (
                      <div className={`${post.isPremium ? 'h-32' : 'h-48'
