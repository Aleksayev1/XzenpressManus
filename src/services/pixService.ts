...
export class MockPixProvider implements PixProvider {
  ...
  async generatePixPayment(data: PixPaymentData): Promise<PixResponse> {
    ...
    return {
      qrCode: mockQrCode,
      qrCodeBase64: await this.generateRealQRCodeBase64(mockQrCode),
      pixKey: mockQrCode,
  ...
  private async generateRealQRCodeBase64(pixCode: string): Promise<string> {
    // Importar dinamicamente a biblioteca qrcode
    const QRCode = (await import('qrcode')).default;

    try {
      // Gerar QR Code real como Data URL (Base64)
      ...
      // Fallback: retornar SVG placeholder se falhar
